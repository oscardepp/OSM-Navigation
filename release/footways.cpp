...
//
  ...
  //
 void Footways::findMin(Building &B,Nodes &nodes, pair<long long, long long> &min,double &minDistance) {
      pair<double, double> avg_loc = B.getLocation(nodes);
      double lat1,long1;
      lat1 = avg_loc.first;
      long1 = avg_loc.second;
      double lat2,long2, distance;
      bool isEntrance;
      for (Footway &F: this->MapFootways) {
        for(long long fid: F.NodeIDs) {
          nodes.find(fid,lat2,long2,isEntrance);
